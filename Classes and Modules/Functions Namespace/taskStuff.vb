    ...
    Module taskStuff
        ...
        Public Sub enableBuiltInRestorePointTask()
            Try
                ...
                taskService.Dispose()
            Catch ex As Exception
                eventLogFunctions.writeToApplicationLogFile("Unable to disable built-in Windows System Restore Task.", EventLogEntryType.Error, False)
        ...
        Public Sub addRunTimeTask(taskName As String, taskDescription As String, taskEXEPath As String, taskParameters As String, Optional boolAllowParallelRunMode As Boolean = False)
            ...
            Try
                ...
                Catch ex As Exception
                    ...
                    eventLogFunctions.writeToApplicationLogFile("There was an error while validating the task definition settings.", EventLogEntryType.Error, False)
                    eventLogFunctions.writeCrashToApplicationLogFile(ex)
                ...
                eventLogFunctions.writeToApplicationLogFile(String.Format("Creating task ""{0}"" in ""{1}"".", taskName, taskFolderObject.Name), EventLogEntryType.Information, False)
        ...
        Public Sub updateScheduledRestorePointCreationTaskWithEverySetting()
                        ...
                        If taskWeAreWorkingWith IsNot Nothing Then
                            If taskWeAreWorkingWith.Definition.Triggers.Count <> 0 Then ' Now we make sure our task has triggers to work with.
                                ...
                                If taskWeAreWorkingWith.Definition.Triggers(0).TriggerType = TaskScheduler.TaskTriggerType.Daily Then
                                    ...
                                    eventLogFunctions.writeToApplicationLogFile("The upgrade of the scheduled task was successful.", EventLogEntryType.Information, False)
                                End If
                            End If
            ...
            Catch ex As Exception
                ...
                eventLogFunctions.writeToApplicationLogFile("An error occurred while upgrading scheduled task.", EventLogEntryType.Error, False)
                eventLogFunctions.writeCrashToApplicationLogFile(ex)
            End Try
        End Sub
        ...
        Private Sub addPrioritySettingsSubRoutine(ByRef task As TaskScheduler.Task)
            Try
                If task.Definition.Settings.Priority <> ProcessPriorityClass.Normal Then
                    ...
                    eventLogFunctions.writeToApplicationLogFile("Setting priority settings for the task named """ & task.Name & """.", EventLogEntryType.Information, False)
                    task.Dispose()
                End If
            Catch ex As Exception
            ...
            Try
                ...
                If doesRunTimeTaskExist("Restore Point Creator -- Run with no UAC", task) Then
                    If task.Definition.Settings.MultipleInstances <> TaskScheduler.TaskInstancesPolicy.Parallel Then
                        ...
                        task.RegisterChanges()
                        task.Dispose()
                        eventLogFunctions.writeToApplicationLogFile("Modified RunTime Task with MultiRun mode enabled.", EventLogEntryType.Information, False)
                    End If
                End If
            Catch ex As Exception
                eventLogFunctions.writeCrashToApplicationLogFile(ex, EventLogEntryType.Warning)
